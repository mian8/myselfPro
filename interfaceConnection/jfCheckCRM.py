...
class jfDemo(baseFunc):
    ...
    def customerid(self, session, code):
        ...
        crmCode=crmCode.get_text().strip().split("：")[-1]
        try:
            superCode=page.find("input",{"id":"customer_supercustid"}).get("value")
            #0表示没有推荐人
            if superCode != "0":
                superphone,superCrmCo = self.getSuperCus(session,superCode)
                logging.debug(f"推荐人内部id{superCode}，推荐人外部id{superCrmCo}，推荐人手机号{superphone}")
                superCus=[superCode,superphone,superCrmCo]
            else:superCus=[]
        except Exception as e:
            logging.warn(e)
            logging.warn(page.find("input", {"id": "customer_supercustid"}))
        return phone.get("value"),crmCode,superCus
    ...
    #通过指定数据源，获取认购单详情
    def fromList(self,crmcode=[],phone=[],houseNum=[]):
        session = self.crm_login("gxcd", "1234567a")
        params = {'searchtime_type': 'subdate', 'startTime': '', 'endTime': '', 'searchType': '6',
                  ...
                  'userid': '959', 'premisesid': '', 'pageSize': '10', 'pageNum': '1'}
        url="http://112.74.129.135:8180/AustralianPlat/admin/dcSubscription/querylist.jhtml"
        if crmcode:
            searchType=6
            keysList = crmcode
        elif phone:
        ...
        for key in  keysList:
            params["keys"]=key
            end = session.post(url,data=params)
            page = BeautifulSoup(end.text, "lxml")
            pageNum = page.find("div",{"class":"select-page"})
            print(pageNum)
            print(pageNum.get_text())

    #列表处理
    def listPageDeal(self, session,page):
        allSub = page.find("tbody")
        thisPage = []
        allTr=allSub.find_all("tr")
        if allTr :
            for line in allTr:
                ...
                if cusMsgInside["usrcode"]:
                    cusMsgInside["phone"], cusMsgInside["crmCode"], cusMsgInside["superCus"] = self.customerid(session,
                                                                                                               cusMsgInside[
                                                                                                                   "usrcode"])
                else:
                    cusMsgInside["crmCode"], cusMsgInside["superCus"] = -1, []
                thisPage.append(cusMsgInside)
                # 添加到表中
                self.saveSubIdLIst(cusMsgInside)
                # print(cusMsgInside)
        Numb = len(allTr)
        return thisPage, Numb
    ...
    def getSubId(self, session, pageSize=10, pageNum=2):
        ...
        thisPage, Numb=self.listPageDeal(session,page)
    ...
    def subdetil(self,session, subId,usrcode,substatusO,statusO):
        ...
        #让我看看谁不一样啊
        if substatusO != allSubstatus[str(substatusI)]:print("这两不一致%s %s"%(substatusO,allSubstatus[str(substatusI)]))
        #房屋交割积分补发
        if substatusI==11:isFdMultiple=1
        else:isFdMultiple=0
        #认购状态
        if statusO in ["退房","转提名","交割违约"]:statusMultiple=0
        elif statusO in ["正常","换房","更名","换房中","退房中"]:statusMultiple=1
        else:statusMultiple=-1
        logging.debug(f"成交状态 {substatusO} 认购状态 {statusO}")
        return isFdMultiple,statusMultiple

    #记录积分到txt
    def saveJfToTxt(self,custmerPoint,superPoint):
        newline=""
        if custmerPoint:
            # newline=newline+str(custmerPoint)+"\n"
            newline=newline+",".join([str(x) for x in custmerPoint])+"\n"
        if superPoint:
            # newline = newline + str(superPoint) + "\n"
            newline=newline+",".join([str(x) for x in superPoint])+"\n"
        address = self.address + "/SubPointLIst.txt"
        if newline:
            with open(address,"a+",encoding="utf8") as file:
                file.write(newline)
    ...
    def mainF(self, name, pwd, SubIdLIst=""):
        ...
        for subId in SubIdLIst:
            ...
            if payStatusMultiple == 1:
                ...
                endPoint = start * statusMultiple * totalJF * payStatusMultiple * premisesTypeMultiple * natureMultiple * queryMultiple
                #交割补齐积分
                followPoint=endPoint * isFdMultiple
                allPoint=endPoint+followPoint
                if premisesTypeMultiple==0.5:
                    logging.info(f"{subId['houseNum']} 应得积分 {allPoint} = 积分档位{totalJF} X 认购状态{statusMultiple} X 首付齐状态{payStatusMultiple} X 项目类型{premisesTypeMultiple} X 房屋性质{natureMultiple} X 优惠折扣{queryMultiple} + 房屋交割{followPoint}")
                elif premisesTypeMultiple==1:
                    logging.info(f"{subId['houseNum']} 应得积分 {allPoint} = 积分档位{totalJF} X 认购状态{statusMultiple} X 首付齐状态{payStatusMultiple} X 项目类型{premisesTypeMultiple} X 房屋性质{natureMultiple} X 优惠折扣{queryMultiple}")

                #积分为0则本单不获得积分
                if allPoint !=0:
                    #认购单积分：认购id，CRMcode，手机号，本单总积分，客户id，客户积分类型
                    custmerPoint = subId['houseNum'], subId["subId"], subId["crmCode"], subId["phone"], allPoint, subId["usrcode"],1
                else:custmerPoint=[]

                #处理推荐人积分
                supPoint = 0
                #有推荐人且认购单为有效成交状态
                if subId["superCus"] and statusMultiple !=0 :
                    # print("有推荐人")
                    #下级客户的优惠折扣决定计算方法
                    if queryMultiple==0:
                        superEnd = start * statusMultiple * totalJF * payStatusMultiple * premisesTypeMultiple * natureMultiple

                        followPoint = superEnd * isFdMultiple
                        #刨除优惠，应得积分的10%
                        supPoint = (superEnd + followPoint)*0.1

                    elif queryMultiple==1 or queryMultiple==0.5:
                        #实际积分的20%
                        supPoint = allPoint*0.2
                    else:pass
                    # 认购单积分：认购id，推荐人CRMcode，推荐人手机号，推荐人本单总积分，推荐人客户id，推荐人积分类型
                    superPoint = subId['houseNum'], subId["subId"], subId["superCus"][2], subId["superCus"][1], supPoint, subId["superCus"][0],2
                else:superPoint=[]
            ...
            self.saveJfToTxt(custmerPoint,superPoint)
...
if __name__ == "__main__":
    ...
    crmcode=["CRM2833384"]
    fool.fromList(crmcode=["CRM2833384"])
